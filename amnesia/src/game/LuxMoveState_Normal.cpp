...
/*
Soma headbob for hpl2 made by timan, painfully imported into x64 by me
(NutBoi)
...
void cLuxMoveState_Normal::UpdateHeadBob(float afTimeStep)
{
	...
	float fHealth = mpPlayer->GetHealth() / 100;
	float fHealthMul = 1.0f - fHealth;
	//head moves too much on low health
	fHealthMul = cMath::Clamp(fHealthMul, 0, 0.5f);
	...
	//mpPlayer->GetCamera()->SetPosition(mvHeadBob * mfHeadBobMul);
	mpPlayer->SetHeadPosAdd(eLuxHeadPosAdd_Bob, mvHeadBob * mfHeadBobMul);
	if(mpPlayer->mfLeanRoll == 0 && mpPlayer->mfRoll == 0)
	{
