...
void cLuxPlayerLantern::SetActive(bool abX, bool abUseEffects, bool abCheckForOilAndItems, bool abCheckIfAllowed)
{
	...
	//HPL jank at the rescue once more
	...
	if(mbActive)
	{
		//it's this easy. not so much for the hands tho.
		mpPlayer->GetHands()->SetActiveHandObject("lantern" + to_string(lanternNum));
		
		msTurnOnSound = gpBase->mpPlayer->GetHands()->GetCurrentHandObject()->msLanternOnSoundS;
		if(abUseEffects) gpBase->mpHelpFuncs->PlayGuiSoundData(msTurnOnSound, eSoundEntryType_Gui);
	}
	else
	{
		msTurnOffSound = gpBase->mpPlayer->GetHands()->GetCurrentHandObject()->msLanternOffSoundS;
		msOutOfOilSound = gpBase->mpPlayer->GetHands()->GetCurrentHandObject()->msLanternOutOfOilSoundS;
