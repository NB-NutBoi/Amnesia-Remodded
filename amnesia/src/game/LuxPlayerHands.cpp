...
//-----------------------------------------------------------------------
//custom made to be the shittiest code, intentionally broken (not really i just suck :munshi_pain:)

void cLuxPlayerHands::UpdateHandModel(int m, cLuxMap* apMap)
{
	mlActiveHand = m;
	
	//don't ask, you don't wanna hear it.

	/*this is the seventh iteration of this code,
	this is the very best i could come up with and it still sucks
	hopefully that gives you an idea of how much i suck.
	*/
	...
	{
		gpBase->mpPlayer->GetHelperLantern()->SetActive(false, true);
		UpdateHandModel(m, apMap);
	}
	else if (mHandState == eLuxHandsState_Holster) 
	{
...
void cLuxPlayerHands::CreateHandEntity(cLuxMap *apMap)
{
	...
	switch (mlActiveHand) {
	case 0:
		apMap->GetWorld()->CreateEntity("PlayerHands", cMatrixf::Identity, "models/player/hands/hands0.ent");
		break;
	case 1:
		apMap->GetWorld()->CreateEntity("PlayerHands", cMatrixf::Identity, "models/player/hands/hands1.ent");
		break;
	case 2:
		apMap->GetWorld()->CreateEntity("PlayerHands", cMatrixf::Identity, "models/player/hands/hands2.ent");
		break;
	case 3:
		apMap->GetWorld()->CreateEntity("PlayerHands", cMatrixf::Identity, "models/player/hands/hands3.ent");
		break;
	case 4:
		apMap->GetWorld()->CreateEntity("PlayerHands", cMatrixf::Identity, "models/player/hands/hands4.ent");
		break;
	case 5:
		apMap->GetWorld()->CreateEntity("PlayerHands", cMatrixf::Identity, "models/player/hands/hands5.ent");
		break;
	default:
		apMap->GetWorld()->CreateEntity("PlayerHands", cMatrixf::Identity, "models/player/hands/hands0.ent");
		break;
