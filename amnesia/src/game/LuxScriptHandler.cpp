...
void cLuxScriptHandler::InitScriptFunctions()
{
	...
	AddFunc("void SetLantern(int alLanternNum)", (void*)SetLantern);
	AddFunc("void SetArm(int alArm)", (void*)SetArm);
	AddFunc("int GetLantern()", (int*)GetLantern);
	AddFunc("int GetArm()", (int*)GetArm);
	...
	AddFunc("void RemoveNote(string &in entry)", (void*)RemoveNote);
	...
	AddFunc("float GetEntityRotationX(string &in asName)", (float*)GetEntityRotationX);
	AddFunc("float GetEntityRotationY(string &in asName)", (float*)GetEntityRotationY);
	AddFunc("float GetEntityRotationZ(string &in asName)", (float*)GetEntityRotationZ);
	AddFunc("float GetCameraRotationX()", (float*)GetCameraRotationX);
	AddFunc("float GetCameraRotationY()", (float*)GetCameraRotationY);
	AddFunc("float GetCameraRotationZ()", (float*)GetCameraRotationZ);
...
void __stdcall cLuxScriptHandler::SetLantern(int alLanternNum)
{
	gpBase->mpPlayer->GetHelperLantern()->lanternNum = alLanternNum;
}

int __stdcall cLuxScriptHandler::GetLantern()
{
	return gpBase->mpPlayer->GetHelperLantern()->lanternNum;
}

void __stdcall cLuxScriptHandler::SetArm(int alArmNum)
{
	gpBase->mpPlayer->GetHands()->UpdateHandModel(alArmNum, gpBase->mpMapHandler->GetCurrentMap());
}
int __stdcall cLuxScriptHandler::GetArm(int alArmNum)
{
	return gpBase->mpPlayer->GetHands()->mlActiveHand;
}
...
void __stdcall cLuxScriptHandler::RemoveNote(string& asEntryName)
{
	gpBase->mpJournal->RemoveNote(asEntryName);
}
...
float __stdcall cLuxScriptHandler::GetEntityRotationX(string& asName) {
	iLuxEntity* pEntity = GetEntity(asName, eLuxEntityType_LastEnum, -1);
	if (pEntity->GetBodyNum() == 0)
	{
	...
	cVector3f fRot = cMath::MatrixToEulerAngles(pEntity->GetBody(0)->GetLocalMatrix(), eEulerRotationOrder_XYZ);

	return fRot.x;
	
}
float __stdcall cLuxScriptHandler::GetEntityRotationY(string& asName) {
	iLuxEntity* pEntity = GetEntity(asName, eLuxEntityType_LastEnum, -1);
	if (pEntity->GetBodyNum() == 0)
	{
		Error("Could not get rotation of entity '%s' because it has no physics body!\n", asName.c_str());
...
float __stdcall cLuxScriptHandler::GetEntityRotationZ(string& asName) {
	iLuxEntity* pEntity = GetEntity(asName, eLuxEntityType_LastEnum, -1);
	if (pEntity->GetBodyNum() == 0)
	{
		Error("Could not get rotation of entity '%s' because it has no physics body!\n", asName.c_str());
...
//--------------------------Setting Entity rotations---------------------------------------------

// not today, my head hurts too much.
